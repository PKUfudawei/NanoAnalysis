...
def fit_signal(year, signal_mass):
    ...
    f_out = ROOT.TFile(f"output/{year}/workspace_signal_{m}.root", "RECREATE")
    ...
    with open(f'output/{year}/fit_info_signal_{m}.yaml', 'w', encoding='utf-8') as f:
        info = {
            'x0': x0.getVal(),
...
def background_fit(year):
    ...
    f_out = ROOT.TFile(f"./output/{year}/workspace_background.root", "RECREATE")
    ...
    with open(f'output/{year}/fit_info_background.yaml', 'w', encoding='utf-8') as f:
        info = {
            #'p0': p0.getVal(),
            'p1': p1.getVal(),
            'p2': p2.getVal(),
...
def get_SR_data(year):
    ...
    plot.Draw()
    can.Update()
    can.SaveAs(f"../plots/fit/{year}/SR_data_Hbb_mass_Zprime.pdf")

    f_out = ROOT.TFile(f"./output/{year}/workspace_data.root", "RECREATE")
...
def get_SR_bkg_MC(year):
    ...
    can = ROOT.TCanvas()
    plot = mass_Zprime.frame()
    mc_SR.plotOn(plot, binning)
    plot.Draw()
    can.Update()
    can.SaveAs(f"../plots/fit/{year}/SR_mc_Hbb_mass_Zprime.pdf")

    f_out = ROOT.TFile(f"./output/{year}/workspace_mc_SR.root", "RECREATE")
    w = ROOT.RooWorkspace("workspace_mc_SR", "workspace_mc_SR")
    getattr(w, "import")(mc_SR)
    w.Print()
    w.Write()
    f_out.Close()
