...
class GenMatch():
    ...
    def update_particle(
        self, name: str, pool: GenParticleArray, cut: ak.Array, flatten: bool = True,
        num: int = 1, variables = {'pt', 'eta', 'phi', 'mass', 'pdgId'},
    ) -> None:  # default particles shape: (event, particle)
        self.particle[name] = ak.pad_none(pool[cut & (ak.sum(cut, axis=1)==num)], target=num, axis=1)  # shape: (event, particle)
    ...
    def top(self) -> np.array:  # single top process
        ...
        self.update_particle(  # select one t or t~
            ...
            ), name='t', num=1, flatten=True, variables=None
        )
        ...
        self.update_particle(  # select one b with the opposite sign as top
            pool=self.childs_of['t'], cut=(
                ...
                self.childs_of['t'].hasFlags(["fromHardProcess"])
            ), name='b', num=1, flatten=True, variables=None
        )
    ...
    def ZpToHGamma(self) -> dict:
        ...
        self.update_particle(  # shape: (event, H)
            pool=self.event.GenPart, cut=(
                ...
                self.event.GenPart.hasFlags(['fromHardProcess', 'isLastCopy'])
            ), name='H', flatten=True, num=1, variables=None
        )
        ...
        self.update_particle(  # shape: (event, W)
            pool=self.event.GenPart, cut=(  # shape: (event, H_childs)
                (abs(self.event.GenPart.pdgId) == self.PDGID['W+']) &
                self.event.GenPart.hasFlags(['fromHardProcess', 'isLastCopy'])
            ), name='W', flatten=True, num=2, variables=None
        )
        
        self.update_particle(  # shape: (event, W_childs)
            pool=self.childs_of['W'], cut=(  # shape: (event, WW)
                self.childs_of['W'].hasFlags(['fromHardProcess'])
            ), name='WW_childs', flatten=True, num=4, variables=None
        )
