...
def parse_commandline():
    ...
    parser.add_argument('-o', '--out_dir', help='To specify the input directory', type=str, default='./datacards/input/')
...
def plot_signal_fit(model, result, fit_variable, mc, signal_region, mass, x_max, x_min, bin_width=50):
    ...
    os.makedirs(f'../postprocess/plots/fit/{mass}', exist_ok=True)
    canvas.SaveAs(f"../postprocess/plots/fit/{mass}/signal_fit_{signal_region}.pdf")


def fit_signal(in_file, out_dir, mass, signal_region, cut, fit_range_low=650, fit_range_high=4000):
    sig_model_dir = os.path.join(out_dir, str(mass))
    ...
    # shape uncertainties
    JES_2016 = ROOT.RooRealVar("CMS_scale_j_2016", "CMS_scale_j_2016", 0, -5, 5)
    JES_2017 = ROOT.RooRealVar("CMS_scale_j_2017", "CMS_scale_j_2017", 0, -5, 5)
    JES_2018 = ROOT.RooRealVar("CMS_scale_j_2018", "CMS_scale_j_2018", 0, -5, 5)
    JER = ROOT.RooRealVar('CMS_res_j_13TeV', 'CMS_res_j_13TeV', 0, -5, 5)
    PES = ROOT.RooRealVar('CMS_scale_g_13TeV', 'CMS_scale_g_13TeV', 0, -5, 5)
    PER = ROOT.RooRealVar('CMS_res_g_13TeV', 'CMS_res_g_13TeV', 0, -5, 5)
    ...
    mean = ROOT.RooFormulaVar("mean", "mean",
        "@0*(1+%f*(0.264*@1+0.301*@2+0.435*@3)+%f*@4)"%(systematics['CMS_scale_j_13TeV'][index]-1, (systematics['CMS_scale_g_13TeV'][index]-1)/2), 
        ROOT.RooArgList(x0, JES_2016, JES_2017, JES_2018, PES))
    widthL = ROOT.RooFormulaVar("widthL", "widthL", 
        "@0*(1+%f*@1+%f*@2)"%(systematics['CMS_res_j_13TeV'][index]-1, systematics['CMS_res_g_13TeV'][index]-1), 
    ...
    signal_norm = {
        year: ROOT.RooRealVar(f"model_bbgamma_{SR}_norm_{year}", f"Number of signal events in {SR} {year}", norm[year], 0, 10*norm[year])
        for year in norm
    }
...
def load_signal_model(signal_region, mass):
    sig_model_dir = os.path.join(args.out_dir, str(mass))
...
def plot_b_only_fit(candidates, model, result, fit_variable, data, region, x_min=650, x_max=4000, bin_width=50, signal_mass=None):
    ...
    if signal_mass is not None:
        ...
        with open(os.path.join(args.out_dir, f'{signal_mass}/{signal_region}.yaml'), 'r') as f:
...
def fit_background(in_file, out_dir, region, cut, fit_range_low=650, fit_range_high=4000, signal_mass=None):
    ...
    # store workspace
    with open(os.path.join(out_dir, f'background_{region}.yaml'), 'w', encoding='utf-8') as f:
        info = {
    ...
    f_out = ROOT.TFile(os.path.join(out_dir, f'data_{region}.root'), "RECREATE")
...
if __name__ == "__main__":
    ...
    for signal_region in signal_regions:
        ...
        for mass in signal_mass:
            fit_signal(
                in_file=os.path.join(args.in_dir, 'Run2', str(mass), f'{signal_region[:3]+signal_region[4:]}.root'), 
                out_dir=args.out_dir, mass=mass, signal_region=signal_region, cut=SR_cut
            )

        if 'SRH' in signal_region:
            fit_background(in_file=os.path.join(args.in_dir, 'Run2', 'data.root'), out_dir=args.out_dir, region=CR, cut=CR_cut)
        if '_N' in signal_region:
            fit_background(in_file=os.path.join(args.in_dir, 'Run2', 'data.root'), out_dir=args.out_dir, region=signal_region.split('_')[0], cut=SR_cut, signal_mass=(1550 if 'SRH' in signal_region else 1750))
