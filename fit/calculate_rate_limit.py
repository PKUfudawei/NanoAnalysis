...
for mass in os.listdir('datacard/'):
    for file in set(os.listdir(f'datacard/{mass}')):
        if not file.endswith('.txt'):
            continue
        os.system(f"combine -M HybridNew datacard/{mass}/{file} -n .{mass}_{file.split('.')[0].replace('Tag', '')} --run blind --setParameters pdfindex_CR1=-1,pdfindex_CR2=-1 --X-rtd REMOVE_CONSTANT_ZERO_POINT=1")
