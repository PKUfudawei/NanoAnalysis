...
for mass in os.listdir('datacard/'):
    for file in set(os.listdir(f'datacard/{mass}')):
        if not file.endswith('.txt'):
            continue
        os.system(f"combine -M AsymptoticLimits datacard/{mass}/{file} -n {mass}_{file.split('.')[0].replace('Tag', '')} --run blind --setParameters pdfindex_CR1=-1,pdfindex_CR2=-1 --freezeParameters pdfindex_CR1,pdfindex_CR2")
