...
def plot_signal_fit(model, result, fit_variable, mc, signal_region, mass, x_max, x_min, bin_width=50):
    ...
    legend = ROOT.TLegend(0.11, 0.7, 0.89, 0.9)
    ...
    frame.GetYaxis().SetTitle('Events / '+str(bin_width)+' GeV')
    frame.Draw()
    legend.Draw()
    ...
    canvas.SaveAs(f"../postprocess/plots/fit/{year}/{mass}/signal_fit_{signal_region}_{mass}.pdf")
